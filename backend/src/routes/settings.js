...
router.put('/', validateSettings, async (req, res) => {
  try {
    ...
    const validationResult = validateSettingsData(newSettings);
...
router.put('/:key', async (req, res) => {
  try {
    ...
    const validationResult = validateSpecificSetting(settingKey, value);
    if (!validationResult.isValid) {
      return res.status(400).json({
        success: false,
...
router.post('/restore', async (req, res) => {
  try {
    ...
    const validationResult = validateSettingsData(settings);
    if (!validationResult.isValid) {
      return res.status(400).json({
        success: false,
...
// Helper function to validate specific setting
function validateSpecificSetting(key, value) {
